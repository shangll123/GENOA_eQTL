...
colnames(res_all) = c("R2_0","R2_001","R2_002","R2_005","R2_01","R2_02","R2_05")
rownames(res_all) = c("AAbslmm" , "EAbslmm", "AAelnet", "EAelnet", "AFAelnet","CAUelnet","HISelnet")
write.csv(res_all,paste0(path_pred,"/result/res_elnet_predict_",popu[k],".1Mb.csv"),quote=F)
res_percent = round(res_all/res_all[,1],3)
write.csv(res_percent,paste0(path_pred,"/result/res_percent_elnet_predict_",popu[k],"1Mb..csv"),quote=F)
}
...
count = count + 1
	load(paste0(path_pred,"/",popnames[count],".RData"))
	R2 = (result$pearson_rho_v2)^2
	R2list[[count]] = na.omit(R2)
	gene_list[[count]] = rownames(R2list[[count]])
}

common_gene = Reduce(intersect,  gene_list) 

for(count in 1:7){
	R2s = R2list[[count]][match(common_gene, rownames(R2list[[count]])),]
	pop = R2s[,k]
	res_all[count,1] = sum(pop>=0)
	res_all[count,2] = sum(pop>=0.01)
	res_all[count,3] = sum(pop>=0.02)
...
colnames(res_all) = c("R2_0","R2_001","R2_002","R2_005","R2_01","R2_02","R2_05")
rownames(res_all) = c("AAbslmm" , "EAbslmm", "AAelnet", "EAelnet", "AFAelnet","CAUelnet","HISelnet")
write.csv(res_all,paste0(path_pred,"/result/res_elnet_predict_",popu[k],".1Mbcommon.csv"),quote=F)
#res_percent = round(res_all/res_all[,1],3)
#write.csv(res_percent,paste0(path_pred,"/result/res_percent_elnet_predict_",popu[k],"1Mbcommon.csv"),quote=F)
}
...
pdf(paste0(path, "/",popu[k],"_scatter_R2_1Mb.pdf"),width=6,height=6)
...
```
# check overlapped genes in better predicted genes across models
```R
#-----------------------
# compare common genes
#-----------------------
...
for(i in 1:7){
	for(j in 1:7){
		overlap_mat[i,j] = length(intersect(list_gene[[i]],list_gene[[j]]))
	}

}

colnames(overlap_mat) = c("AAbslmm" , "EAbslmm", "AAelnet", "EAelnet", "AFAelnet","CAUelnet","HISelnet")
rownames(overlap_mat) = c("AAbslmm" , "EAbslmm", "AAelnet", "EAelnet", "AFAelnet","CAUelnet","HISelnet")
write.csv(overlap_mat,paste0(path_pred,"/result/overlap_at_0.1_",popu[k],".1Mbcommon.csv"),quote=F)


colnames(res_all) = c("R2_0","R2_001","R2_002","R2_005","R2_01","R2_02","R2_05")
rownames(res_all) = c("AAbslmm" , "EAbslmm", "AAelnet", "EAelnet", "AFAelnet","CAUelnet","HISelnet")
write.csv(res_all,paste0(path_pred,"/result/res_elnet_predict_",popu[k],".1Mbcommon_update.csv"),quote=F)
